...
class PhotoPermissionHandler {
  static Future<bool> requestPhotoPermission(BuildContext context) async {
    // Use storage permission for Android, photos for iOS
    ...
    if (status.isPermanentlyDenied) {
      ...
      final bool shouldOpenSettings = await showDialog(
        ...
        builder: (BuildContext context) => AlertDialog(
          ...
          content: Text(
            'Storage access is needed to upload your profile picture, ID, and clearance documents. '
    ...
    if (!status.isGranted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Storage access is required to upload images. '
...
class _SignUpPageState extends State<SignUpPage> {
  ...
  if (kIsWeb) {
    ...
    // Use the new permission handler here
